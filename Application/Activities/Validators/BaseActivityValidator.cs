        ...
        : AbstractValidator<T> where TDto : BaseActivityDto
{
    ...
    {
        RuleFor(x => selector(x).Title).NotEmpty().WithMessage("Title is requied")
           .MaximumLength(100).WithMessage("Title must not exceeed 100 characters");
        RuleFor(x => selector(x).Description).NotEmpty().WithMessage("Description is requied");
